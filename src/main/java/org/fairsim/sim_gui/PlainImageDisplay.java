...
public class PlainImageDisplay {
    ...
    static class HistogramDisplay extends JComponent {
        ...
        private final int pointCounter = 0;
        ...
        void setData(float[] dat, float min, float max) {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < nrBins; x++) {
                    ...
                    } else if (logCount[x] * height > (height - y - 1) * maxLogCount) {
    ...
    public interface IUpdate {
        void newZoom(int level, int xPos, int yPos);
    }
            ...
            for (int i = 0; i < 100; i++) {
                for (int y = 0; y < height; y++)
                    for (int x = 0; x < width; x++) {
                        ...
                        } else if ((x > 400 && x < 450) || (y > 250 && y < 290)) {

...
public class SimParam implements Vec2d.Size {
    ...
    private final int stackSize = -1;                // z-stack length in #slices
    private final double micronsPerSlice = -1;            // spatial extent of the z-slice stage shift
    private final double cyclesPerMicronInZ = -1;        // freq extent of FFT in z-slices
    ...
    public class Dir {
        ...
        private final double[] phases;   // phases
        private final double[] modul;    // modulation
        private final int thisBand;        // which band is this
        ...
        private final double[] phaseIntensityFactors; // the intensity factors to apply phase-by-phase
    ...
    public void saveConfig(Conf.Folder cfg) {
        ...
        for (int d = 0; d < nrDirs; d++) {
            ...
            df.newDbl("shift").setVal(dir.pX * (dir.nrBands - 1), dir.pY * (dir.nrBands - 1));
    ...
    public enum IMGSEQ {
        ...
        public int calcPos(int ang, int pha, int z, int angMax, int phaMax, int zMax) {
            ...
            switch (this) {
                case PZA:
                    return (pha + z * phaMax + ang * phaMax * zMax);
                case ZAP:
                    return (z + ang * zMax + pha * zMax * angMax);
                case PAZ:
                default:
        ...
        public SimParam getParam() {
            switch (this) {
                case PZA:
                    return SimParam.create(3, 3, 5, 512, 0.082, null);
                case ZAP:
                    return SimParam.create(3, 5, 5, 1024, 0.064, null);
                default:
                    return null;
            }
        }
